...
import { Spot, UserConfigValue } from '../../core/domain/spots/Spot';
import {
  BlockedUsersCallback,
  NextSongsIdsCallback,
  RemoteSpotsService,
  SpotConfigCallback,
} from '../../core/domain/spots/SpotsService';
...
export default class FirebaseSpotsService implements RemoteSpotsService {
  ...
  async fetchSpot(spotId: string): Promise<Spot> {
    ...
    const spotRef = realTimeDatabase.ref(path);
  ...
  async getPopularSpots(): Promise<Spot[]> {
    const spotsRef = realTimeDatabase.ref('spots');
  ...
  setOnlineStatus(spotId: string, isOnline: boolean): Promise<void> {
    const isOnlineRef = realTimeDatabase.ref(`/spots/${spotId}/isOnline`);
  ...
  private updateSpotRef(spotId: string) {
    if (!this.spotRef || this.currentSpotId !== spotId)
      this.spotRef = realTimeDatabase.ref(`spots/${spotId}`);
  ...
  subscribeToSpotConfigUpdates(
    ...
    const configRef = realTimeDatabase.ref(`spots/${spotId}/config`);
  ...
  addSongToNextSongsIds(spotId: string, songId: string): Promise<void> {
    return realTimeDatabase.ref(`spots/${spotId}/nextSongsIds`).update({
      [songId]: true,
    });
  }

  clearNextSongIds(spotId: string): Promise<void> {
    return realTimeDatabase.ref(`spots/${spotId}/nextSongsIds`).remove();
  }
